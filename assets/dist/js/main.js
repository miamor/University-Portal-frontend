...
function loadNews(content, cat, $div) {
    $.getJSON(API_URL + '/posts/cat/' + cat, function (response) {
        console.log(response);
    })
}

function renderContent($a, _Posts) {
    $a.find('[for]').each(function () {
        $thisLoop = $(this);
        loopVar = $(this).attr('for').split('=')[0];
        rx = /{([^}]+)}/g;
        loopD = rx.exec($(this).attr('for').split('=')[1])[1];
        loopStart = loopD.split(' to ')[0];
        ...
        while (j <= loopEnd) {
            if (_Posts[j]) {
                ...
                while (curMatch = rxp.exec(loopContent)) {
                    ...
                    attribute = v.split('.')[1];
                    //console.log(v+' ~ '+_Posts[j][attribute]);
            ...
            loopContents.push(loopContent);
            j++;
        }
        //console.log(loopContents);

        $thisLoop.html(loopContents.join(''));
    });
}
    ...
    while (curMatch = rxp.exec(response.content)) {
        ...
        if (v.indexOf(':') > -1) { // contains :
            ...
            if ($.inArray(catid, Cats) == -1) {
                ...
                $.getJSON(API_URL + '/posts/cats/' + catid, function (postscat) {
                    News[catid] = postscat;
                    renderContent($this, postscat);
                });
            } else {
                //$('body').html(JSON.stringify(News[catid]));
                //renderContent($this, News[catid]);
            }
        } else {
...
function loadModules() {
    $('.load_module').each(function () {
        moduleID = $(this).attr('attr-module');
        //console.log(moduleID);
        $this = $(this);
        // get full content of this module
        $.getJSON(API_URL + '/modules/' + moduleID, function (response) {
            //console.log(response);
